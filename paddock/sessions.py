...
def get_session(ref: str) -> Session | None:
    """Find a session by name or by id — the chooser shows names, scripts use ids."""
    for session in list_sessions():
        if ref in (session.name, session.session_id):
            return session
    return None
...
def _record(session: Session) -> None:
    """Store the session as given: a flag set on it — keep_alive — is written back with it."""
    live = list_sessions()
...
def _save(sessions: list[Session]) -> None:
    ...
    temp.write_text(json.dumps([asdict(session) for session in sessions], indent=2) + "\n")
